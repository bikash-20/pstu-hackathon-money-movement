...
We are using **PostgreSQL** (hosted on Render's free tier).
...
Instead, we maintain a `balance` column on the `users` table and an immutable `transactions` table. This allows us to demonstrate ACID properties: wrapping the deduction, the addition, and the log insertion in a single atomic database transaction.
